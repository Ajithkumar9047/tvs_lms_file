...
# Print the differences with brief explanations
print("Differences:")
for key, value in differences.items():
    if key == "values_changed":
        for change_key, change_value in value.items():
            path = change_key[0]
            explanation = change_value["new_value"]
            print(f"At path '{path}': {explanation}")